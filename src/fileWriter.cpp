...
void FileWriter::write_solution_file(vector<Cell> grid, Config config, double time, string filename) {
    int rk_step = 0;
    std::ofstream outfile(config.get_base_path() + filename + to_string(file_counter_) + ".out");

    if (!outfile.is_open()) {
        std::cerr << "Error opening file: " << config.get_base_path() + filename + to_string(file_counter_) + ".out" << std::endl;
        return;
    }
    cout<<"Successfully created a output file : "<<config.get_base_path() + filename + to_string(file_counter_) + ".out" <<endl;
